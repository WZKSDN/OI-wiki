...
首先选择长度最小的字符串 $S_1$ ，而后构造 $S_1$ 的后缀自动机。将 $S_2, S_3,\ldots,S_k$ 逐一字符在后缀自动机上转移（参照上一题的做法），对后缀自动机的每一状态记录在这一状态最长的匹配：这一个状态结束的 $S_1$ 和 $S_i$ 的最长公共子串的长度，并在后缀链接 $link$ 构成的树上进行更新：每一个节点的最长匹配为其子树内的最长匹配和这个节点所代表长度最长串长度中较小的一个值。

而后，对于后缀自动机的每一状态，在这一状态的最长匹配即是其对 $S_2, S_3, \ldots, S_k$ 匹配的最小值。对所有最小值取其中最大的一个，即是答案。

这个做法的复杂度为 $O(|S_1|*k + \sum_{i=1}^k |S_i|)$ ，由于字符串 $S_1$ 是最短的，复杂度即是 $O( \sum_{i=1}^k |S_i|) $。
